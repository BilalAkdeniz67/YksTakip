...
function processStoreData(parsed) {
  ...
  if (!parsed.studyPlan || typeof parsed.studyPlan !== "object") parsed.studyPlan = {};
  if (!parsed.studySessions || typeof parsed.studySessions !== "object") parsed.studySessions = {};
...
function normalizeDayMap(map) {
  const out = {};
  Object.entries(map || {}).forEach(([date, items]) => {
    out[date] = Array.isArray(items) ? items : Object.values(items || {});
  });
  return out;
}
...
function studentSummary(student) {
  return {
    ...
    weekStudyMinutes: student.weekStudyMinutes || calcWeekStudyMinutes(student.id)
  };
}
...
async function loadStore() {
  ...
  if (activeProfile.role === "student") {
    ...
    store.studySessions = { [student.id]: normalizeDayMap(data.studySessions) };
    store.studyPlan = normalizeDayMap(data.studyPlan);
    student.weekStudyMinutes = calcWeekStudyMinutes(student.id);
    ...
    const selfIndex = store.students.findIndex((item) => String(item.id) === String(student.id));
    if (selfIndex >= 0) store.students[selfIndex] = { ...store.students[selfIndex], ...student };
    ...
    await Promise.all(summaries.map(async (student) => {
      ...
      student.weekStudyMinutes = data.weekStudyMinutes || 0;
    }));
  }
}
  ...
  if (store.currentUser.role === "student" && student) {
    await Promise.all([
      ...
      dbRef.child(`studentData/${student.id}`).set({
        ...
        studySessions: store.studySessions[student.id] || {},
        studyPlan: store.studyPlan || {},
        weekStudyMinutes: calcWeekStudyMinutes(student.id)
      }),
...
function setRole(role, userId) {
  ...
  if (role === "student") {
    ...
    $("swFloat")?.classList.remove("hidden");
    renderStopwatch();
  } else {
...
function showPage(page) {
  ...
  const hints = {
    ...
    reports: "Raporların",
    rank: "Sıralama",
    calendar: "Aylık soru takvimi",
    "study-calendar": "Çalışma planı ve kronometre",
    "teacher-home": "Sınıf özeti",
    "teacher-rank": "Liderlik tablosu",
...
let teacherRankMode = "questions";

function weekStudyMinutesOf(student) {
  if (student?.weekStudyMinutes) return Number(student.weekStudyMinutes) || 0;
  return calcWeekStudyMinutes(student.id);
}

function renderRankList(targetId, ranked, mode, youId) {
  const el = $(targetId);
  if (!el) return;
  const isStudy = mode === "study";
  el.innerHTML = ranked
    .map((x, i) => {
      const isYou = youId && String(x.id) === String(youId);
      const badge = i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `${i + 1}`;
      const score = isStudy ? fmtMinutes(x.totalMinutes || 0) : `${x.total} Soru`;
      const color = isStudy ? "#6366f1" : "var(--orange)";
      return `
        <li class="${isYou ? "you" : ""}" style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; margin-bottom: 8px;">
          ...
          <div style="flex: 1;">
            ...
            ${x.classId ? `<small style="display: block; color: var(--muted); font-size: 12px;">${x.classId}</small>` : ""}
          </div>
          <b style="color: ${color}; font-size: 15px;">${score}</b>
        </li>`;
    })
    .join("");
}
...
function renderRank() {
  ...
  if ($("rankTabStudy")) {
    $("rankTabStudy").classList.toggle("active", studentRankMode === "study");
  }

  const listData = studentRankMode === "school"
    ? store.students
    : store.students.filter((x) => x.classId === s.classId);

  if (studentRankMode === "study") {
    const ranked = listData
      .map((x) => ({ ...x, totalMinutes: weekStudyMinutesOf(x) }))
      .sort((a, b) => b.totalMinutes - a.totalMinutes);
    renderRankList("rankList", ranked, "study", s.id);
    return;
  }
  ...
  renderRankList("rankList", ranked, "questions", s.id);
}

function renderTeacherRank() {
  if ($("teacherRankTabQuestions")) {
    $("teacherRankTabQuestions").classList.toggle("active", teacherRankMode === "questions");
  }
  if ($("teacherRankTabStudy")) {
    $("teacherRankTabStudy").classList.toggle("active", teacherRankMode === "study");
  }

  if (teacherRankMode === "study") {
    const ranked = store.students
      .map((x) => ({ ...x, totalMinutes: weekStudyMinutesOf(x) }))
      .sort((a, b) => b.totalMinutes - a.totalMinutes)
      .slice(0, 20);
    renderRankList("teacherRankList", ranked, "study", null);
    return;
  }
  ...
  const ranked = store.students
    ...
    .sort((a, b) => b.total - a.total)
    .slice(0, 20);
  renderRankList("teacherRankList", ranked, "questions", null);
}
...
function renderCalendar(forStudentId = null, isTeacherView = false) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const studyMins = dayStudyMinutes(sId, dateStr);
    if (studyMins > 0) {
      const studyBadge = document.createElement("div");
      studyBadge.className = "cal-study-badge";
      studyBadge.textContent = fmtMinutes(studyMins);
      cell.appendChild(studyBadge);
    }
...
// ===== STUDY CALENDAR & STOPWATCH HELPERS =====
...
function planHours(item) {
  if (item?.plannedHours != null && item.plannedHours !== "") return Number(item.plannedHours) || 0;
  return (Number(item?.minutes) || 0) / 60;
}

function planMinutes(item) {
  return Math.round(planHours(item) * 60);
}

function dayStudySeconds(studentId, dateStr) {
  const dayArr = ((store.studySessions || {})[studentId] || {})[dateStr] || [];
  return (Array.isArray(dayArr) ? dayArr : []).reduce((sum, sess) => sum + (sess.seconds || 0), 0);
}

function dayStudyMinutes(studentId, dateStr) {
  const histMins = Number(((store.history[studentId] || {})[dateStr] || {}).studyMinutes) || 0;
  const fromSessions = Math.round(dayStudySeconds(studentId, dateStr) / 60);
  return Math.max(histMins, fromSessions);
}

function calcWeekStudyMinutes(studentId) {
  const sessions = (store.studySessions || {})[studentId] || {};
  const now = new Date();
  const day = now.getDay();
  const monday = new Date(now);
  monday.setDate(now.getDate() - (day === 0 ? 6 : day - 1));
  monday.setHours(0, 0, 0, 0);
  const history = (store.history || {})[studentId] || {};
  const dates = new Set([...Object.keys(sessions), ...Object.keys(history)]);
  let total = 0;
  dates.forEach((dateStr) => {
    const d = new Date(`${dateStr}T00:00:00`);
    if (d >= monday && d <= now) {
      // Use the daily aggregate so several short sessions are counted together
      // and older history-only records remain part of the weekly ranking.
      total += dayStudyMinutes(studentId, dateStr);
    }
  });
  return total;
}
...
function formatStudyDate(dateStr) {
  ...
  return `${Number(day)} ${MONTH_NAMES_TR[Number(month) - 1]} ${year}`;
}

function studentStudySessions(studentId) {
  const dayMap = (store.studySessions || {})[studentId] || {};
  return Object.entries(dayMap).flatMap(([date, sessions]) => {
    const list = Array.isArray(sessions) ? sessions : Object.values(sessions || {});
    return list.map((session, index) => ({ ...session, date, index }));
  }).sort((a, b) => {
    const aTime = Number(a.ts) || new Date(`${a.date}T00:00:00`).getTime() + a.index;
    const bTime = Number(b.ts) || new Date(`${b.date}T00:00:00`).getTime() + b.index;
    return bTime - aTime;
  });
}

function totalStudyMinutes(studentId) {
  const sessions = (store.studySessions || {})[studentId] || {};
  const history = (store.history || {})[studentId] || {};
  return [...new Set([...Object.keys(sessions), ...Object.keys(history)])]
    .reduce((total, date) => total + dayStudyMinutes(studentId, date), 0);
}

function renderTeacherStudyDetail(studentId) {
  const totalElement = $("profStudyTotal");
  const subjectsElement = $("profStudySubjects");
  const sessionsElement = $("profStudySessions");
  if (!subjectsElement || !sessionsElement) return;

  const sessions = studentStudySessions(studentId);
  const totalMinutes = totalStudyMinutes(studentId);
  if (totalElement) totalElement.textContent = fmtMinutes(totalMinutes);

  const bySubject = sessions.reduce((totals, session) => {
    const subject = session.subject || "Diğer";
    totals[subject] = (totals[subject] || 0) + (Number(session.seconds) || 0);
    return totals;
  }, {});
  const rankedSubjects = Object.entries(bySubject).sort(([, a], [, b]) => b - a);
  const highestSeconds = rankedSubjects[0]?.[1] || 1;
  subjectsElement.innerHTML = rankedSubjects.length
    ? rankedSubjects.map(([subject, seconds]) => `
        <div class="prof-subject-row">
          <div class="prof-subject-meta"><strong>${escapeHtml(subject)}</strong><span>${fmtTime(seconds)}</span></div>
          <div class="prof-subject-track"><span style="width:${Math.max(5, Math.round((seconds / highestSeconds) * 100))}%"></span></div>
        </div>`).join("")
    : `<p class="prof-empty-note">Henüz ders bazında çalışma oturumu kaydedilmedi.</p>`;

  sessionsElement.innerHTML = sessions.length
    ? sessions.map((session) => `
        <article class="prof-session-item">
          <div>
            <p class="prof-session-subject">${escapeHtml(session.subject || "Diğer")}</p>
            <p class="prof-session-topic">${escapeHtml(session.topic || "Konu girilmedi")}</p>
            <p class="prof-session-date">${formatStudyDate(session.date)}</p>
          </div>
          <strong>${fmtTime(Number(session.seconds) || 0)}</strong>
        </article>`).join("")
    : `<p class="prof-empty-note">Öğrencinin ayrıntılı çalışma oturumu bulunmuyor.</p>`;
}

const MONTH_NAMES_TR = ["Ocak", "Şubat", "Mart", "Nisan", "Mayıs", "Haziran", "Temmuz", "Ağustos", "Eylül", "Ekim", "Kasım", "Aralık"];
let scCalMonth = todayDateObj.getMonth();
let scCalYear = todayDateObj.getFullYear();
let scSelectedDate = todayDateStr;

function renderStudyCalendar() {
  const s = currentStudent();
  if (!s) return;
  if ($("scCalMonthYear")) $("scCalMonthYear").textContent = `${MONTH_NAMES_TR[scCalMonth]} ${scCalYear}`;
  const grid = $("scCalendarGrid");
  if (!grid) return;
  Array.from(grid.children).forEach((child) => {
    if (!child.classList.contains("cal-day-name")) child.remove();
  });
  const firstDay = new Date(scCalYear, scCalMonth, 1).getDay();
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    const dateStr = `${scCalYear}-${String(scCalMonth + 1).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
    const cell = document.createElement("div");
    cell.className = "cal-day";
    cell.dataset.date = dateStr;
    cell.dataset.forSc = "true";
    if (dateStr === todayDateStr) cell.style.borderColor = "var(--orange)";
    if (dateStr === scSelectedDate) cell.classList.add("active");
    const num = document.createElement("div");
    num.className = "cal-num";
    num.textContent = d;
    cell.appendChild(num);

    const dayPlan = Array.isArray(plan[dateStr]) ? plan[dateStr] : [];
    dayPlan.slice(0, 3).forEach((item) => {
      const chip = document.createElement("div");
      chip.className = "cal-plan-chip";
      const hours = planHours(item);
      const hoursLabel = Number.isInteger(hours) ? String(hours) : hours.toFixed(1);
      chip.textContent = `${item.subject}: ${hoursLabel} sa`;
      chip.title = chip.textContent;
      cell.appendChild(chip);
    });
    if (dayPlan.length > 3) {
      const more = document.createElement("div");
      more.className = "cal-plan-chip";
      more.textContent = `+${dayPlan.length - 3}`;
      cell.appendChild(more);
    }

    const totalMins = dayStudyMinutes(s.id, dateStr);
    if (totalMins > 0) {
      const sb = document.createElement("div");
      sb.className = "cal-study-badge";
      sb.textContent = fmtMinutes(totalMins);
      cell.appendChild(sb);
    }
    grid.appendChild(cell);
  }
  if ($("spDate") && !$("spDate").value) $("spDate").value = todayDateStr;
  renderStudyPlanList(scSelectedDate);
}

function renderStudyPlanList(dateStr) {
  const s = currentStudent();
  if (!s) return;
  const targetDate = dateStr || $("spDate")?.value || todayDateStr;
  const detailCard = $("scDetailCard");
  const detailDate = $("scDetailDate");
  const detailContent = $("scDetailContent");
  const planList = $("studyPlanList");
  if (detailDate) {
    const [y, m, d] = targetDate.split("-");
    detailDate.textContent = `${parseInt(d, 10)} ${MONTH_NAMES_TR[parseInt(m, 10) - 1]} ${y}`;
  }
  if (detailCard) detailCard.classList.remove("hidden");

  const daySessions = ((store.studySessions || {})[s.id] || {})[targetDate] || [];
  const history = (store.history[s.id] || {})[targetDate] || {};
  if (detailContent) {
    const questionSummary = `<p style="font-size:16px;font-weight:700;color:var(--navy);margin-bottom:10px;">Çözülen soru: <strong>${history.solved || 0}</strong></p>`;
    if (daySessions.length) {
      const totalSecs = daySessions.reduce((sum, sess) => sum + (sess.seconds || 0), 0);
      detailContent.innerHTML = questionSummary + `<p style="font-size:16px;font-weight:700;color:#6366f1;margin-bottom:10px;">Toplam çalışma: <strong>${fmtTime(totalSecs)}</strong></p>` +
        daySessions.map((sess) => `
          <div class="study-session-item">
            <div>
              <div class="ss-subject">${sess.subject}</div>
              ${sess.topic ? `<div class="ss-topic">${sess.topic}</div>` : ""}
            </div>
            <div class="ss-time">${fmtTime(sess.seconds || 0)}</div>
          </div>`).join("");
    } else {
      detailContent.innerHTML = questionSummary + `<p style="color:var(--muted);">Bu tarih için henüz süre kaydı yok.</p>`;
    }
  }

  const plans = (store.studyPlan || {})[targetDate] || [];
  if (planList) {
    planList.innerHTML = plans.length
      ? `<p style="font-weight:700;color:var(--navy);margin:8px 0;">Planlanan dersler</p>` +
        plans.map((p, i) => `
          <div class="study-plan-item">
            <div class="sp-label">${p.subject}</div>
            <div style="display:flex;align-items:center;gap:10px;">
              <div class="sp-time">${fmtMinutes(planMinutes(p))}</div>
              <button type="button" class="btn-ghost" data-del-plan="${targetDate}|${i}" style="font-size:12px;padding:4px 10px;">Sil</button>
            </div>
          </div>`).join("")
      : `<p style="color:var(--muted);">Bu güne plan eklenmedi.</p>`;
  }
}

function renderStudyDayDetail(dateStr) {
  scSelectedDate = dateStr;
  renderStudyPlanList(dateStr);
}
...
function setFocusMode(active, requestFullscreen = false) {
  document.body.classList.toggle("focus-mode", active);
  const overlay = $("focusOverlay");
  if (overlay) overlay.classList.toggle("hidden", !active);

  // Fullscreen can only be requested directly from a user action, so failures are harmless.
  if (active && requestFullscreen && document.documentElement.requestFullscreen && !document.fullscreenElement) {
    document.documentElement.requestFullscreen().then(() => {
      focusFullscreenActive = true;
    }).catch(() => {});
  }
  if (!active && focusFullscreenActive && document.fullscreenElement && document.exitFullscreen) {
...
function syncStopwatch() {
  if (swRunning && swStartedAt) {
    swSeconds = Math.floor((Date.now() - swStartedAt) / 1000);
  }
}

function renderStopwatch() {
  syncStopwatch();
  const disp = fmtTime(swSeconds);
  if ($("swDisplay")) $("swDisplay").textContent = disp;
  if ($("swFloatTime")) $("swFloatTime").textContent = disp;
  if ($("focusTime")) $("focusTime").textContent = disp;
  const state = swRunning ? "running" : swPaused ? "paused" : "idle";
  const isStudent = store.currentUser?.role === "student";
  const dot = $("swStatusDot");
  const display = $("swDisplay");
  const startBtn = $("swStartBtn");
  const pauseBtn = $("swPauseBtn");
  const saveBtn = $("swSaveBtn");
  const floatWidget = $("swFloat");
  const floatStart = $("swFloatStart");
  const floatPause = $("swFloatPause");
  const floatStop = $("swFloatStop");
  if (dot) dot.className = `sw-status-dot${state === "running" ? " running" : state === "paused" ? " paused" : ""}`;
  if (display) display.className = `sw-display${state === "running" ? " running" : state === "paused" ? " paused" : ""}`;
  if (startBtn) startBtn.style.display = state === "idle" ? "" : "none";
  if (pauseBtn) {
    pauseBtn.style.display = state !== "idle" ? "" : "none";
    pauseBtn.textContent = state === "paused" ? "▶ Devam" : "⏸ Duraklat";
  }
  if (saveBtn) saveBtn.style.display = state !== "idle" ? "" : "none";
  if (floatStart) floatStart.style.display = state === "idle" ? "" : "none";
  if (floatPause) {
    floatPause.style.display = state !== "idle" ? "" : "none";
    floatPause.textContent = state === "paused" ? "▶" : "⏸";
  }
  if (floatStop) floatStop.style.display = state !== "idle" ? "" : "none";
  if (floatWidget) {
    floatWidget.classList.toggle("hidden", !isStudent);
    const subject = $("swSubject")?.value || "Kronometre";
    if ($("swFloatLabel")) $("swFloatLabel").textContent = state === "idle" ? "Kronometre" : subject;
    if ($("focusSubject")) $("focusSubject").textContent = subject;
  }
  setFocusMode(swRunning);
}
...
function startTimer() {
  ...
  swRunning = true;
  swPaused = false;
  swStartedAt = Date.now() - (swSeconds * 1000);
  swInterval = setInterval(() => {
    renderStopwatch();
  }, 1000);
  renderStopwatch();
  setFocusMode(true, true);
  if ($("swNote")) $("swNote").textContent = "";
}
...
function saveStudySession(subject, topic, seconds) {
  const s = currentStudent();
  if (!s || seconds < 1) return null;
  if (!store.studySessions) store.studySessions = {};
  if (!store.studySessions[s.id]) store.studySessions[s.id] = {};
  const existing = store.studySessions[s.id][todayDateStr];
  if (!Array.isArray(existing)) store.studySessions[s.id][todayDateStr] = Object.values(existing || {});
  const entry = { subject, topic, seconds, ts: Date.now() };
  store.studySessions[s.id][todayDateStr].push(entry);
  const totalMins = Math.max(1, Math.round(seconds / 60));
  if (!store.history[s.id]) store.history[s.id] = {};
  const prev = store.history[s.id][todayDateStr] || { solved: 0 };
  store.history[s.id][todayDateStr] = {
    ...prev,
    studyMinutes: (prev.studyMinutes || 0) + totalMins
  };
  s.weekStudyMinutes = calcWeekStudyMinutes(s.id);
  saveStore();
  return entry;
}
...
function stopTimer() {
  ...
  swStartedAt = null;
  if (swSeconds < 5) {
    if ($("swNote")) $("swNote").textContent = "Çok kısa süre — kaydedilmedi (en az 5 sn).";
    swSeconds = 0;
    renderStopwatch();
    return;
  }
  const subject = $("swSubject")?.value || "Diğer";
  const topic = ($("swTopic")?.value || "").trim();
  const seconds = swSeconds;
  const saved = saveStudySession(subject, topic, seconds);
  swSeconds = 0;
  renderStopwatch();
  if (saved && $("swNote")) $("swNote").textContent = `${subject} — ${fmtTime(seconds)} kaydedildi.`;
...
$("rankTabStudy")?.addEventListener("click", () => {
  studentRankMode = "study";
  renderRank();
});
$("teacherRankTabQuestions")?.addEventListener("click", () => {
  teacherRankMode = "questions";
  renderTeacherRank();
});
$("teacherRankTabStudy")?.addEventListener("click", () => {
  teacherRankMode = "study";
  renderTeacherRank();
});
$("goStudyCalendarBtn")?.addEventListener("click", () => {
  document.querySelectorAll("#studentNav .nav-btn").forEach((b) => {
    b.classList.toggle("active", b.dataset.page === "study-calendar");
  });
  showPage("study-calendar");
});

$("scCalPrev")?.addEventListener("click", () => {
  scCalMonth--;
  if (scCalMonth < 0) {
    scCalMonth = 11;
    scCalYear--;
...
$("scCalNext")?.addEventListener("click", () => {
  scCalMonth++;
  if (scCalMonth > 11) {
    scCalMonth = 0;
    scCalYear++;
...
$("studyPlanForm")?.addEventListener("submit", (event) => {
  ...
  const dateStr = $("spDate")?.value || todayDateStr;
  const subject = $("spSubject")?.value;
  const hours = Number($("spHours")?.value);
  if (!subject || !hours || hours <= 0) return;
  if (!store.studyPlan) store.studyPlan = {};
  if (!Array.isArray(store.studyPlan[dateStr])) store.studyPlan[dateStr] = Object.values(store.studyPlan[dateStr] || {});
  store.studyPlan[dateStr].push({ subject, plannedHours: hours });
  if ($("spHours")) $("spHours").value = "";
  scSelectedDate = dateStr;
  saveStore();
  renderStudyCalendar();
});
...
$("swStartBtn")?.addEventListener("click", startTimer);
$("swPauseBtn")?.addEventListener("click", pauseTimer);
$("swSaveBtn")?.addEventListener("click", stopTimer);
$("swFloatStart")?.addEventListener("click", startTimer);
$("swFloatPause")?.addEventListener("click", pauseTimer);
$("swFloatStop")?.addEventListener("click", stopTimer);
...
$("solveForm")?.addEventListener("submit", (event) => {
  ...
  const prev = store.history[student.id][todayDateStr] || {};
...
document.body.addEventListener("click", (event) => {
  ...
  if (profCalDayBtn && activeProfileStudentId) {
    ...
    if (hist) {
      ...
      const studyMins = dayStudyMinutes(activeProfileStudentId, dateStr);
      if (studyMins > 0) {
        html += `<p style="margin-top:8px;color:#6366f1;font-weight:700;">Çalışma süresi: ${fmtMinutes(studyMins)}</p>`;
      }
      const sessions = ((store.studySessions || {})[activeProfileStudentId] || {})[dateStr] || [];
      if (sessions.length) {
        html += sessions.map((sess) => `<p style="color:var(--muted);font-size:13px;">${sess.subject}${sess.topic ? " · " + sess.topic : ""} — ${fmtTime(sess.seconds || 0)}</p>`).join("");
      }
      $("profCalDetailContent").innerHTML = html;
    } else {
      const studyMins = dayStudyMinutes(activeProfileStudentId, dateStr);
      $("profCalDetailContent").innerHTML = studyMins
        ? `<p style="color:#6366f1;font-weight:700;">Çalışma süresi: ${fmtMinutes(studyMins)}</p>`
        : `<p style="color: var(--muted);">Bu tarih için kayıt bulunmuyor.</p>`;
    }
  }

  const scDayBtn = event.target.closest("#scCalendarGrid .cal-day:not(.empty)");
  if (scDayBtn) {
    document.querySelectorAll("#scCalendarGrid .cal-day").forEach((el) => el.classList.remove("active"));
    scDayBtn.classList.add("active");
    const dateStr = scDayBtn.dataset.date;
    if ($("spDate")) $("spDate").value = dateStr;
    renderStudyDayDetail(dateStr);
    return;
  }

  const delPlanBtn = event.target.closest("[data-del-plan]");
  if (delPlanBtn) {
    const [dateStr, index] = delPlanBtn.dataset.delPlan.split("|");
    if (Array.isArray(store.studyPlan?.[dateStr])) {
      store.studyPlan[dateStr].splice(Number(index), 1);
      if (!store.studyPlan[dateStr].length) delete store.studyPlan[dateStr];
      saveStore();
      renderStudyCalendar();
    }
    return;
  ...
  const calDayBtn = event.target.closest("#calendarGrid .cal-day:not(.empty)");
  if (calDayBtn) {
    ...
    const studyMins = dayStudyMinutes(s.id, dateStr);
    const sessions = ((store.studySessions || {})[s.id] || {})[dateStr] || [];
    if (hist || studyMins) {
      let html = `<p style="font-size: 18px; margin-bottom: 8px;"><strong>${hist?.solved || 0}</strong> Soru Çözüldü</p>`;
      if (hist?.details) {
        html += `<p style="color: var(--muted);">${hist.details}</p>`;
