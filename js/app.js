...
const firebaseConfig = {
  apiKey: "AIzaSyCN1bw1mVygE9uHbElZOXQvS1rnMIo3-kE",
  ...
  appId: "1:428783558644:web:67c06811b83ae3c3b223a1",
  measurementId: "G-S7EYVS2C97"
};
...
if (typeof firebase !== "undefined" && firebaseConfig.databaseURL && !firebaseConfig.databaseURL.includes("YOUR_PROJECT_ID")) {
  try {
    ...
    console.log("Firebase Realtime Database aktif ve bağlı!");
  } catch (err) {
...
$("registerForm")?.addEventListener("submit", (e) => {
  ...
  if (store.students.find(s => s.id === id)) {
    $("regError").textContent = "Bu numara ile kayıtlı öğrenci zaten var!";
    return;
  }

  store.students.push({
    id, name, classId, field, password, avatar: "", week: [0, 0, 0, 0, 0, 0, 0]
  });
  saveStore();
